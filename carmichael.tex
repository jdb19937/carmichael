...
$\log L \ge \tfrac{99}{100}\,T\log z \ge \ell_2 \ell_3$ for large $n$, the
...
some exponent $E \in (0, 1/2]$ rather than every $E < 5/12$, so the
machine uses $y = \lceil z^{1-E} \rceil$ for that $E$, and
Lemma~\ref{lem:pigeon} at $B = 21/100$ (the least value for which
$L = x^{1/5} \le x^{B}$), so the scan bound becomes $k \le x^{79/100}$.
...
proof above, not about the formalization. Two intermediate layers are
...
Chebyshev estimate used in Section~4 are all proved inside the
