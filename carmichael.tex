...
$q \in (z^{99/100},\, z]$ with $P^+(q-1) \le y$. Let $Q$ be the $T$
...
\label{rem:lean}
The theorem has been machine-verified in Lean~4 over Mathlib, in
operation-count form: taking Theorems 3 and 3.1 of \cite{AGP} as
hypotheses (their effective constants as parameters), the kernel-checked
formalization proves that for every $\varepsilon > 0$ and all large $n$ a
Carmichael number $m \in (n, n^{1+\varepsilon}]$ exists together with
its complete prime factorization, and that an explicit operation budget
mirroring the five steps of Section~\ref{sec:algo}, with every primality
test charged as trial division, is at most $e^{100\,\ell_2\ell_3}$.
Korselt's criterion, Lemma~\ref{lem:mertens}, Lemma~\ref{lem:vebk} (to
our knowledge the first mechanization of the van Emde Boas--Kruyswijk
bound in any proof assistant), and the Chebyshev estimate used in
Section 4 are all proved inside the formalization, so the two cited
theorems of \cite{AGP} are its only unproved inputs. The formalization is
