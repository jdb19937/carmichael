/-
Extraction inputs at windowed scales (Route T).

The windowed transliteration of `extraction_inputsE` (`ExtractionE.lean`):
for any scales in the window `InWindow C₁ E n z w y T` and any
`Q ⊆ goodPrimesW z w y` of size `T`, the modulus is at least `2`, `N* ≥ 1`,
...
Boas–Kruyswijk). The scale facts come from the window; the asymptotic
comparison `star_boundW` is `star_boundE` with the window slack
(`z ≤ 4 C₁ ℓ₂ ℓ₃`, `y ≤ 4 z^{1-E}`, `T ≤ 5 ℓ₂`) absorbed into the constants:
`(y+1) log z ≤ 48 C₁ ℓ₂^{1-E} ℓ₃² + 3 ℓ₃ ≤ 0.01 ℓ₂` eventually.
-/
...
private lemma exists_le_of_le_mapW {α β : Type*} [DecidableEq α] [DecidableEq β] (f : α → β) :
...
private lemma pool_mem_factsW {Q : Finset ℕ} {z k p : ℕ} (hp : p ∈ pool Q z k) :
...
private lemma pool_coprimeW {Q : Finset ℕ} {z k p : ℕ}
    (hQ : ∀ q ∈ Q, q ≤ z ∧ q.Prime) (hp : p ∈ pool Q z k) :
    p.Coprime (Lmod Q) := by
  obtain ⟨hpp, hzp, -⟩ := pool_mem_factsW hp
...
private lemma exponent_dvd_lambdaW {Q : Finset ℕ} (hQp : ∀ q ∈ Q, q.Prime)
    (hL : 2 ≤ Lmod Q) :
...
private lemma lambdaL_ne_zeroW {Q : Finset ℕ} (hQp : ∀ q ∈ Q, q.Prime) :
    lambdaL Q ≠ 0 := by
  classical
...
private lemma round_extractW {L : ℕ} (hL : 2 ≤ L) {lam : ℕ}
  ...
  obtain ⟨u', hu'le, hu'map⟩ := exists_le_of_le_mapW f t W.attach.val htle
...
private lemma lambdaL_le_powW {Q : Finset ℕ} {y z : ℕ} (hz : 1 ≤ z)
...
private lemma log_Lmod_geW {Q : Finset ℕ} {z : ℕ} (hz : 1 ≤ z)
...
private lemma log_Lmod_leW {Q : Finset ℕ} {z : ℕ}
...
/-- `N* ≤ λ(L)(1 + log L) + 2`. -/
private lemma Nstar_leW (Q : Finset ℕ) :
...
/-- `λ(L)(1 + log L) < N*`. -/
private lemma lt_NstarW (Q : Finset ℕ) :
...
private lemma natLog_le_divW {L n : ℕ} (hn : 1 ≤ n) {c : ℝ} (hc : 0 < c)
...
private lemma tendsto_ell2_atTopW : Tendsto (fun n : ℕ => ell2 n) atTop atTop :=
  Real.tendsto_log_atTop.comp (Real.tendsto_log_atTop.comp tendsto_natCast_atTop_atTop)

private lemma tendsto_ell3_atTopW : Tendsto (fun n : ℕ => ell3 n) atTop atTop :=
  Real.tendsto_log_atTop.comp tendsto_ell2_atTopW

/-- `16 x² ≤ e^{0.03 x}` for `x ≥ 10⁸` (the constant `16` absorbs
`1 + 15 ℓ₂ ℓ₃ ≤ 16 ℓ₂²` from `T ≤ 5 ℓ₂`, `log z ≤ 3 ℓ₃`). -/
private lemma eventually_sq_le_expW :
    ∀ᶠ x : ℝ in atTop, 16 * x ^ 2 ≤ Real.exp (0.03 * x) := by
...
/-- The core asymptotic comparison at windowed scales: for any `z, y, T`
with `C₁ ℓ₂ ℓ₃ ≤ z ≤ 4 C₁ ℓ₂ ℓ₃`, `y ≤ 4 z^{1-E}`, `T ≤ 5 ℓ₂`, the number of
pool elements consumed by the greedy loop, bounded via the smoothness
estimate for `λ(L)` and the round-count estimate, is eventually below
`(log n)^{1.2} = e^{1.2 ℓ₂}`. The key point is
`(y + 1) log z ≤ 48 C₁ ℓ₂^{1-E} ℓ₃² + 3 ℓ₃ ≤ 0.01 ℓ₂` eventually, from
`ℓ₃² ≤ ℓ₂^{E/4}` (`eventually_log_sq_le_rpow`). Only the window upper
bounds enter; the eventual hypotheses are on `ℓ₂ n`, `ℓ₃ n` alone. -/
private lemma star_boundW (C₁ E : ℝ) (hE : 0 < E) (hE2 : E ≤ 1 / 2)
    (h1000 : (1000 : ℝ) ≤ C₁) :
    ∀ᶠ n : ℕ in atTop, ∀ z y T : ℕ,
      C₁ * ell2 n * ell3 n ≤ (z : ℝ) → (z : ℝ) ≤ 4 * (C₁ * ell2 n * ell3 n) →
      (y : ℝ) ≤ 4 * (z : ℝ) ^ ((1 : ℝ) - E) → (T : ℝ) ≤ 5 * ell2 n →
      (Real.exp (ell2 n) / (1.5 * ell2 n * ell3 n) + 1) *
          ((z : ℝ) ^ (y + 1) * (1 + (T : ℝ) * Real.log z) + 2)
        ≤ Real.exp (1.2 * ell2 n) := by
  have hE4 : (0:ℝ) < E / 4 := by linarith
  have h3E4 : (0:ℝ) < 3 * E / 4 := by linarith
  have h1E4 : (0:ℝ) < 1 - E / 4 := by linarith
  filter_upwards [tendsto_ell2_atTopW.eventually_ge_atTop 50,
    tendsto_ell3_atTopW.eventually_ge_atTop 1,
    tendsto_ell3_atTopW.eventually_ge_atTop (Real.log (4 * C₁)),
    tendsto_ell2_atTopW.eventually (eventually_log_sq_le_rpow hE4),
    ((tendsto_rpow_atTop h3E4).comp
      tendsto_ell2_atTopW).eventually_ge_atTop (9600 * C₁),
    ((tendsto_rpow_atTop h1E4).comp
      tendsto_ell2_atTopW).eventually_ge_atTop 1200,
    tendsto_ell2_atTopW.eventually eventually_sq_le_expW] with n ha50 hb1 hbC hbsq' h34' h14' hsq'
  intro z y T hzlow hzup hy_up hT_up
  set a := ell2 n with hadef
  set b := ell3 n with hbdef
  have hba : b = Real.log a := rfl
  have hbsq : b ^ 2 ≤ a ^ (E / 4) := by rw [hba]; exact hbsq'
  have h34 : 9600 * C₁ ≤ a ^ (3 * E / 4) := h34'
  have h14 : (1200:ℝ) ≤ a ^ (1 - E / 4) := h14'
  have hsq : 16 * a ^ 2 ≤ Real.exp (0.03 * a) := hsq'
  clear_value a b
  have ha0 : (0:ℝ) < a := by linarith
  have hb0 : (0:ℝ) < b := by linarith
  have ha1 : (1:ℝ) ≤ a := by linarith
  -- z bounds (from the window)
  ...
  have hzpos : (0:ℝ) < (z : ℝ) := lt_of_lt_of_le (by linarith) hzlow
  -- log z bounds
  have hCb : (1000:ℝ) * 1 ≤ C₁ * b := mul_le_mul h1000 hb1 (by norm_num) (by linarith)
  have ha_le_Cab : a ≤ C₁ * a * b := by nlinarith [hCb, ha0]
  have hlogz_low : b ≤ Real.log (z : ℝ) := by
    rw [hba]
    exact Real.log_le_log ha0 (le_trans ha_le_Cab hzlow)
  have hlogz_pos : 0 < Real.log (z : ℝ) := lt_of_lt_of_le hb0 hlogz_low
  have hlogz_up : Real.log (z : ℝ) ≤ 3 * b := by
    calc Real.log (z : ℝ) ≤ Real.log (4 * C₁ * (a * b)) := by
          apply Real.log_le_log hzpos
          calc (z : ℝ) ≤ 4 * (C₁ * a * b) := hzup
            _ = 4 * C₁ * (a * b) := by ring
      _ = Real.log (4 * C₁) + (Real.log a + Real.log b) := by
          rw [Real.log_mul (ne_of_gt (show (0:ℝ) < 4 * C₁ by linarith))
      ...
      _ = 3 * b := by ring
  -- z^{1-E} ≤ 4C₁ · a^{1-E} · b
  have hz1E : (z : ℝ) ^ ((1:ℝ) - E) ≤ 4 * C₁ * (a ^ ((1:ℝ) - E) * b) := by
    have hz2 : (z : ℝ) ≤ 4 * C₁ * (a * b) := by linarith
    have hb1E : b ^ ((1:ℝ) - E) ≤ b := by
      calc b ^ ((1:ℝ) - E) ≤ b ^ (1:ℝ) :=
          Real.rpow_le_rpow_of_exponent_le hb1 (by linarith)
        _ = b := Real.rpow_one b
    have hC1E : (4 * C₁) ^ ((1:ℝ) - E) ≤ 4 * C₁ := by
    ...
    calc (z : ℝ) ^ ((1:ℝ) - E)
      ...
      _ = (4 * C₁) ^ ((1:ℝ) - E) * (a ^ ((1:ℝ) - E) * b ^ ((1:ℝ) - E)) := by
      ...
      _ ≤ 4 * C₁ * (a ^ ((1:ℝ) - E) * b) := by
          ...
          have h3 : (0:ℝ) ≤ (4 * C₁) ^ ((1:ℝ) - E) := Real.rpow_nonneg (by linarith) _
          calc (4 * C₁) ^ ((1:ℝ) - E) * (a ^ ((1:ℝ) - E) * b ^ ((1:ℝ) - E))
              ≤ (4 * C₁) ^ ((1:ℝ) - E) * (a ^ ((1:ℝ) - E) * b) :=
  ...
  -- key bound: (y + 1) log z ≤ 0.01 a
  have hb2 : b ≤ b ^ 2 := by nlinarith
  have hterm1 : 48 * C₁ * (a ^ ((1:ℝ) - E) * b ^ 2) ≤ 0.005 * a := by
    have ht1 : a ^ ((1:ℝ) - E) * b ^ 2 ≤ a ^ ((1:ℝ) - E) * a ^ (E / 4) :=
      mul_le_mul_of_nonneg_left hbsq (Real.rpow_nonneg ha0.le _)
    have h48C : 48 * C₁ ≤ 0.005 * a ^ (3 * E / 4) := by linarith
    calc 48 * C₁ * (a ^ ((1:ℝ) - E) * b ^ 2)
        ≤ 48 * C₁ * (a ^ ((1:ℝ) - E) * a ^ (E / 4)) :=
          mul_le_mul_of_nonneg_left ht1 (by linarith)
      _ = 48 * C₁ * a ^ (1 - 3 * E / 4) := by rw [hsum1]
      _ ≤ (0.005 * a ^ (3 * E / 4)) * a ^ (1 - 3 * E / 4) :=
          mul_le_mul_of_nonneg_right h48C (Real.rpow_nonneg ha0.le _)
      _ = 0.005 * a := by rw [mul_assoc, hsplit1]
  have hterm2 : 3 * b ≤ 0.005 * a := by
    have hbE : b ≤ a ^ (E / 4) := le_trans hb2 hbsq
    have h6 : (6:ℝ) ≤ 0.005 * a ^ (1 - E / 4) := by linarith
    calc 3 * b ≤ 6 * a ^ (E / 4) := by
          have := Real.rpow_nonneg ha0.le (E / 4)
          linarith
      _ ≤ (0.005 * a ^ (1 - E / 4)) * a ^ (E / 4) :=
          mul_le_mul_of_nonneg_right h6 (Real.rpow_nonneg ha0.le _)
      _ = 0.005 * a := by rw [mul_assoc, hsplit2]
  have hkey : ((y : ℝ) + 1) * Real.log (z : ℝ) ≤ 0.01 * a := by
    have h1 : (y : ℝ) + 1 ≤ 16 * C₁ * (a ^ ((1:ℝ) - E) * b) + 1 := by
      linarith
    have hnnB : (0:ℝ) ≤ 16 * C₁ * (a ^ ((1:ℝ) - E) * b) + 1 := by
      have h : (0:ℝ) ≤ 16 * C₁ * (a ^ ((1:ℝ) - E) * b) :=
        mul_nonneg (show (0:ℝ) ≤ 16 * C₁ by linarith)
          (mul_nonneg (Real.rpow_nonneg ha0.le ((1:ℝ) - E)) hb0.le)
      linarith
    have h2 : ((y : ℝ) + 1) * Real.log (z : ℝ) ≤
        (16 * C₁ * (a ^ ((1:ℝ) - E) * b) + 1) * (3 * b) :=
      mul_le_mul h1 hlogz_up hlogz_pos.le hnnB
    have hexpand : (16 * C₁ * (a ^ ((1:ℝ) - E) * b) + 1) * (3 * b) =
        48 * C₁ * (a ^ ((1:ℝ) - E) * b ^ 2) + 3 * b := by ring
    calc ((y : ℝ) + 1) * Real.log (z : ℝ)
        ≤ (16 * C₁ * (a ^ ((1:ℝ) - E) * b) + 1) * (3 * b) := h2
      _ = 48 * C₁ * (a ^ ((1:ℝ) - E) * b ^ 2) + 3 * b := hexpand
  ...
  have hb_le_a : b ≤ a := by
    ...
    linarith
  have hTlogz : (T : ℝ) * Real.log (z : ℝ) ≤ 5 * a * (3 * b) :=
    mul_le_mul hT_up hlogz_up hlogz_pos.le (by linarith)
  have hone_plus : 1 + (T : ℝ) * Real.log (z : ℝ) ≤ 16 * a ^ 2 := by
    nlinarith [hTlogz, mul_le_mul_of_nonneg_left hb_le_a ha0.le, ha50]
  have hfac_nn : (0:ℝ) ≤ 1 + (T : ℝ) * Real.log (z : ℝ) := by
    have := mul_nonneg (Nat.cast_nonneg T : (0:ℝ) ≤ (T : ℝ)) hlogz_pos.le
    linarith
  have hzy_exp : (z : ℝ) ^ (y + 1) =
      Real.exp (((y + 1 : ℕ) : ℝ) * Real.log (z : ℝ)) := by
    rw [← Real.rpow_natCast (z : ℝ) (y + 1),
      Real.rpow_def_of_pos hzpos, mul_comm]
  have hzy_le : (z : ℝ) ^ (y + 1) ≤ Real.exp (0.01 * a) := by
    rw [hzy_exp]
    apply Real.exp_le_exp.mpr
    push_cast
    exact hkey
  have hP2 : (z : ℝ) ^ (y + 1) *
      (1 + (T : ℝ) * Real.log (z : ℝ)) + 2 ≤ Real.exp (0.1 * a) := by
    have hfac_le : 1 + (T : ℝ) * Real.log (z : ℝ) ≤ Real.exp (0.03 * a) :=
  ...
  have hP1nn : (0:ℝ) ≤ Real.exp a / (1.5 * a * b) + 1 := by
    ...
    linarith
  have hP2nn : (0:ℝ) ≤ (z : ℝ) ^ (y + 1) *
      (1 + (T : ℝ) * Real.log (z : ℝ)) + 2 := by
    have := mul_nonneg (pow_nonneg hzpos.le (y + 1)) hfac_nn
    linarith
  calc (Real.exp a / (1.5 * a * b) + 1) *
      ((z : ℝ) ^ (y + 1) *
        (1 + (T : ℝ) * Real.log (z : ℝ)) + 2)
...
/-- The two inputs of the algorithm's extraction loop at windowed scales
(Route T): the modulus is at least `2`, `N* ≥ 1`, every pool element is a
prime `≤ x` coprime to `L`, the pool supplies `⌊log_{L+1} n⌋ + 1` rounds of
`N*` elements, and every `N*`-subset of the pool contains a nonempty subset
with product `≡ 1 (mod L)` (van Emde Boas–Kruyswijk). -/
theorem extraction_inputsW (C₁ E : ℝ) (hE : 0 < E) (hE2 : E ≤ 1 / 2)
    (h1000 : (1000 : ℝ) ≤ C₁) :
    ∀ᶠ n : ℕ in Filter.atTop, ∀ z w y T : ℕ, InWindow C₁ E n z w y T →
      ∀ Q ⊆ goodPrimesW z w y, Q.card = T →
      ∀ k : ℕ, 0 < k → k.Coprime (Lmod Q) →
      (Real.log n) ^ (1.2 : ℝ) ≤ ((pool Q z k).card : ℝ) →
      2 ≤ Lmod Q ∧ 1 ≤ Nstar Q ∧
      (∀ p ∈ pool Q z k, p.Prime ∧ p ≤ xceil Q ∧ p.Coprime (Lmod Q)) ∧
      (Nat.log (Lmod Q + 1) n + 1) * Nstar Q ≤ (pool Q z k).card ∧
      ∀ W ⊆ pool Q z k, W.card = Nstar Q →
        ∃ S ⊆ W, S.Nonempty ∧ (∏ p ∈ S, p) ≡ 1 [MOD Lmod Q] := by
  classical
  filter_upwards [eventually_ge_atTop 2,
    tendsto_ell2_atTopW.eventually_ge_atTop 50,
    tendsto_ell3_atTopW.eventually_ge_atTop 1,
    star_boundW C₁ E hE hE2 h1000] with n hn2 ha50 hb1 hstar
  intro z w y T hW Q hQsub hQcard k hk hkcop hpool
  obtain ⟨hz_lo, hz_hi, hw_lo, hw_hi, hy_lo, hy_hi, hT_lo, hT_hi⟩ := hW
  set a := ell2 n with hadef
  set b := ell3 n with hbdef
  have hba : b = Real.log a := rfl
  clear_value a b
  ...
  have hCab : (1000:ℝ) * 50 * 1 ≤ C₁ * a * b :=
    mul_le_mul hCa hb1 (by norm_num) (le_trans (by norm_num) hCa)
  have hz_real : C₁ * a * b ≤ (z : ℝ) := hz_lo
  have h1z_lt : (1:ℝ) < (z : ℝ) := lt_of_lt_of_le (by linarith) hz_real
  have hz1 : 1 ≤ z := by
    have h : (1:ℝ) ≤ (z : ℝ) := h1z_lt.le
    exact_mod_cast h
  have hCb : (1000:ℝ) * 1 ≤ C₁ * b := mul_le_mul h1000 hb1 (by norm_num) (by linarith)
  have ha_le_Cab : a ≤ C₁ * a * b := by nlinarith [hCb, ha0]
  have hlogz_low : b ≤ Real.log (z : ℝ) := by
  ...
  -- T facts (from the window)
  have hT3a : 3 * a ≤ (T : ℝ) := hT_lo
  have hT1 : 1 ≤ T := by
    have h : (1:ℝ) ≤ (T : ℝ) := by linarith
    exact_mod_cast h
  -- the asymptotic comparison at these scales
  have hstar' := hstar z y T hz_lo hz_hi hy_hi hT_hi
  -- facts about the primes of Q
  have hQfacts : ∀ q ∈ Q, q.Prime ∧ q ≤ z ∧
      Real.sqrt z < (q : ℝ) ∧ SmoothUpTo y (q - 1) := by
    intro q hq
    have hmem := hQsub hq
    rw [goodPrimesW, Finset.mem_filter, Finset.mem_range] at hmem
    refine ⟨hmem.2.1, by omega, ?_, hmem.2.2.2⟩
    have hwq : w + 1 ≤ q := hmem.2.2.1
    have hwqR : (w : ℝ) + 1 ≤ (q : ℝ) := by exact_mod_cast hwq
    calc Real.sqrt (z : ℝ)
        = (z : ℝ) ^ ((1 : ℝ) / 2) := Real.sqrt_eq_rpow _
      _ < (z : ℝ) ^ ((99 : ℝ) / 100) :=
          Real.rpow_lt_rpow_of_exponent_lt h1z_lt (by norm_num)
      _ ≤ (w : ℝ) + 1 := hw_lo
      _ ≤ (q : ℝ) := hwqR
  ...
  have hlogL_ge : 1.5 * a * b ≤ Real.log (Lmod Q) := by
    have h := log_Lmod_geW hz1 (fun q hq => (hQfacts q hq).2.2.1)
    rw [hQcard] at h
    calc 1.5 * a * b = (3 * a) * (b / 2) := by ring
      _ ≤ (T : ℝ) * (Real.log (z : ℝ) / 2) := by
  ...
  -- λ and N* bounds
  have hlam_le : (lambdaL Q : ℝ) ≤ (z : ℝ) ^ (y + 1) := by
    have h := lambdaL_le_powW hz1
      (fun q hq => ⟨(hQfacts q hq).1, (hQfacts q hq).2.1, (hQfacts q hq).2.2.2⟩)
    exact_mod_cast h
  have hlogL_le : Real.log (Lmod Q) ≤ (T : ℝ) * Real.log (z : ℝ) := by
    have h := log_Lmod_leW (fun q hq => (hQfacts q hq).2.1)
      (fun q hq => (hQprime q hq).one_lt.le)
    rwa [hQcard] at h
  have hNstar_le : (Nstar Q : ℝ) ≤ (z : ℝ) ^ (y + 1) *
      (1 + (T : ℝ) * Real.log (z : ℝ)) + 2 := by
    refine le_trans (Nstar_leW Q) ?_
  ...
  have hbudget : (Nat.log (Lmod Q + 1) n + 1) * Nstar Q ≤ (pool Q z k).card := by
    ...
    have hcast : (((Nat.log (Lmod Q + 1) n + 1) * Nstar Q : ℕ) : ℝ) ≤
      ...
      calc ((Nat.log (Lmod Q + 1) n : ℝ) + 1) * (Nstar Q : ℝ)
        ...
        _ ≤ ((pool Q z k).card : ℝ) := hpool
    exact_mod_cast hcast
  -- one round of extraction is always possible
  have hQ_le_z : ∀ q ∈ Q, q ≤ z ∧ q.Prime :=
    fun q hq => ⟨(hQfacts q hq).2.1, (hQfacts q hq).1⟩
  have hextract : ∀ W ⊆ pool Q z k, W.card = Nstar Q →
      ∃ S ⊆ W, S.Nonempty ∧ (∏ p ∈ S, p) ≡ 1 [MOD Lmod Q] := by
    intro W hW hWcard
    refine round_extractW hL2 (exponent_dvd_lambdaW hQprime hL2) (lambdaL_ne_zeroW hQprime)
      (fun p hp => pool_coprimeW hQ_le_z (hW hp)) ?_
    rw [hWcard]
    exact lt_NstarW Q
  ...
  have hN1 : 1 ≤ Nstar Q := by
    ...
    omega
  exact ⟨hL2, hN1,
    fun p hp => ⟨(pool_mem_factsW hp).1, (pool_mem_factsW hp).2.2, pool_coprimeW hQ_le_z hp⟩,
    hbudget, hextract⟩
