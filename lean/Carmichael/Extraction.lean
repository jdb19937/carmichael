/-
Lemma 4.3 of the paper (Step 4 always finds a subset, and the pool
suffices). Proved from the van Emde Boas--Kruyswijk theorem
(`Carmichael/ZeroSum.lean`) by the round-by-round extraction argument.
-/
...
private lemma eventually_log_le_rpow :
    ∀ᶠ x : ℝ in atTop, Real.log x ≤ x ^ ((1 : ℝ) / 8) := by
  filter_upwards [(isLittleO_log_rpow_atTop (by norm_num : (0:ℝ) < 1 / 8)).eventuallyLE,
    eventually_ge_atTop (0 : ℝ)] with x hx hx0
  rw [Real.norm_eq_abs, Real.norm_eq_abs] at hx
  calc Real.log x ≤ |Real.log x| := le_abs_self _
    _ ≤ |x ^ ((1 : ℝ) / 8)| := hx
    _ = x ^ ((1 : ℝ) / 8) := abs_of_nonneg (Real.rpow_nonneg hx0 _)
...
/-- The core asymptotic comparison: the number of pool elements consumed by
the greedy loop, bounded via the smoothness estimate for `λ(L)` and the
round-count estimate, is eventually below `(log n)^{1.2} = e^{1.2 ℓ₂}`. -/
private lemma star_bound :
    ∀ᶠ n : ℕ in atTop,
      (Real.exp (ell2 n) / (1.5 * ell2 n * ell3 n) + 1) *
          ((zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
            (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n)) + 2)
        ≤ Real.exp (1.2 * ell2 n) := by
  have hC : (1000 : ℝ) ≤ A.C₁ := A.thousand_le_C₁
  set K : ℝ := 100 * (3 * (2 * A.C₁) ^ ((2:ℝ)/3) + 6) with hKdef
  filter_upwards [tendsto_ell2_atTop.eventually_ge_atTop 50,
    tendsto_ell3_atTop.eventually_ge_atTop 1,
    tendsto_ell3_atTop.eventually_ge_atTop (Real.log (2 * A.C₁)),
    tendsto_ell2_atTop.eventually eventually_log_le_rpow,
    ((tendsto_rpow_atTop (by norm_num : (0:ℝ) < 1 / 8)).comp
      tendsto_ell2_atTop).eventually_ge_atTop K,
    tendsto_ell2_atTop.eventually eventually_sq_le_exp] with n ha50 hb1 hbC hb8' hK8' hsq'
  set a := ell2 n with hadef
  set b := ell3 n with hbdef
  have hba : b = Real.log a := rfl
  have hb8 : b ≤ a ^ ((1:ℝ)/8) := hb8'
  have hK8 : K ≤ a ^ ((1:ℝ)/8) := hK8'
  have hsq : 13 * a ^ 2 ≤ Real.exp (0.03 * a) := hsq'
  have hzdef : zscale A.C₁ n = ⌈A.C₁ * a * b⌉₊ := rfl
  have hTdef : Tscale n = ⌈(3:ℝ) * a⌉₊ := rfl
  have hydef : yscale A.C₁ n = ⌈(zscale A.C₁ n : ℝ) ^ ((2:ℝ)/3)⌉₊ := rfl
  clear_value a b
  have ha0 : (0:ℝ) < a := by linarith
  have hb0 : (0:ℝ) < b := by linarith
  have ha1 : (1:ℝ) ≤ a := by linarith
  -- z bounds
  have hCa : (1000:ℝ) * 50 ≤ A.C₁ * a := mul_le_mul hC ha50 (by norm_num) (by linarith)
  have hCab : (1000:ℝ) * 50 * 1 ≤ A.C₁ * a * b :=
    mul_le_mul hCa hb1 (by norm_num) (le_trans (by norm_num) hCa)
  have hCab1 : (1:ℝ) ≤ A.C₁ * a * b := by linarith
  have hzlow : A.C₁ * a * b ≤ (zscale A.C₁ n : ℝ) := by
    rw [hzdef]
    exact Nat.le_ceil _
  have hzpos : (0:ℝ) < (zscale A.C₁ n : ℝ) := lt_of_lt_of_le (by linarith) hzlow
  have hzup : (zscale A.C₁ n : ℝ) ≤ 2 * (A.C₁ * a * b) := by
    rw [hzdef]
    have h := Nat.ceil_lt_add_one (show (0:ℝ) ≤ A.C₁ * a * b by linarith)
    linarith
  -- log z bounds
  have hCb : (1000:ℝ) * 1 ≤ A.C₁ * b := mul_le_mul hC hb1 (by norm_num) (by linarith)
  have ha_le_Cab : a ≤ A.C₁ * a * b := by nlinarith [hCb, ha0]
  have hlogz_low : b ≤ Real.log (zscale A.C₁ n : ℝ) := by
    rw [hba]
    exact Real.log_le_log ha0 (le_trans ha_le_Cab hzlow)
  have hlogz_pos : 0 < Real.log (zscale A.C₁ n : ℝ) := lt_of_lt_of_le hb0 hlogz_low
  have hlogb : Real.log b ≤ b := by
    have := Real.log_le_sub_one_of_pos hb0
    linarith
  have hlogz_up : Real.log (zscale A.C₁ n : ℝ) ≤ 3 * b := by
    calc Real.log (zscale A.C₁ n : ℝ) ≤ Real.log (2 * A.C₁ * (a * b)) := by
          apply Real.log_le_log hzpos
          calc (zscale A.C₁ n : ℝ) ≤ 2 * (A.C₁ * a * b) := hzup
            _ = 2 * A.C₁ * (a * b) := by ring
      _ = Real.log (2 * A.C₁) + (Real.log a + Real.log b) := by
  ...
  have hT_up : (Tscale n : ℝ) ≤ 4 * a := by
    ...
    linarith
  -- y bound
  have hy_up : (yscale A.C₁ n : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ ((2:ℝ)/3) + 1 := by
    rw [hydef]
    exact (Nat.ceil_lt_add_one (Real.rpow_nonneg hzpos.le _)).le
  -- rpow arithmetic
  have h78 : a ^ ((7:ℝ)/8) * a ^ ((1:ℝ)/8) = a := by
    rw [← Real.rpow_add ha0]
    norm_num
  have hab : a * b ≤ a ^ ((9:ℝ)/8) := by
    calc a * b ≤ a * a ^ ((1:ℝ)/8) := mul_le_mul_of_nonneg_left hb8 ha0.le
      _ = a ^ ((9:ℝ)/8) := by
        nth_rewrite 1 [← Real.rpow_one a]
        rw [← Real.rpow_add ha0]
        norm_num
  have hz23 : (zscale A.C₁ n : ℝ) ^ ((2:ℝ)/3) ≤ (2 * A.C₁) ^ ((2:ℝ)/3) * a ^ ((3:ℝ)/4) := by
    have hz2 : (zscale A.C₁ n : ℝ) ≤ 2 * A.C₁ * (a * b) := by linarith
    calc (zscale A.C₁ n : ℝ) ^ ((2:ℝ)/3) ≤ (2 * A.C₁ * (a * b)) ^ ((2:ℝ)/3) :=
        Real.rpow_le_rpow hzpos.le hz2 (by norm_num)
      _ = (2 * A.C₁) ^ ((2:ℝ)/3) * (a * b) ^ ((2:ℝ)/3) :=
        Real.mul_rpow (by positivity) (by positivity)
      _ ≤ (2 * A.C₁) ^ ((2:ℝ)/3) * a ^ ((3:ℝ)/4) := by
        apply mul_le_mul_of_nonneg_left _ (Real.rpow_nonneg (by positivity) _)
        calc (a * b) ^ ((2:ℝ)/3) ≤ (a ^ ((9:ℝ)/8)) ^ ((2:ℝ)/3) :=
            Real.rpow_le_rpow (by positivity) hab (by norm_num)
          _ = a ^ ((3:ℝ)/4) := by
            rw [← Real.rpow_mul ha0.le]
            norm_num
  -- key bound: (y + 1) log z ≤ 0.01 a
  have h34b : a ^ ((3:ℝ)/4) * b ≤ a ^ ((7:ℝ)/8) := by
    calc a ^ ((3:ℝ)/4) * b ≤ a ^ ((3:ℝ)/4) * a ^ ((1:ℝ)/8) :=
        mul_le_mul_of_nonneg_left hb8 (by positivity)
      _ = a ^ ((7:ℝ)/8) := by
        rw [← Real.rpow_add ha0]
        norm_num
  have hb78 : b ≤ a ^ ((7:ℝ)/8) := by
    calc b ≤ a ^ ((1:ℝ)/8) := hb8
      _ ≤ a ^ ((7:ℝ)/8) := Real.rpow_le_rpow_of_exponent_le ha1 (by norm_num)
  have hkey : ((yscale A.C₁ n : ℝ) + 1) * Real.log (zscale A.C₁ n : ℝ) ≤ 0.01 * a := by
    have h1 : (yscale A.C₁ n : ℝ) + 1 ≤ (2 * A.C₁) ^ ((2:ℝ)/3) * a ^ ((3:ℝ)/4) + 2 := by
      linarith
    have h2 : ((yscale A.C₁ n : ℝ) + 1) * Real.log (zscale A.C₁ n : ℝ) ≤
        ((2 * A.C₁) ^ ((2:ℝ)/3) * a ^ ((3:ℝ)/4) + 2) * (3 * b) := by
      apply mul_le_mul h1 hlogz_up hlogz_pos.le
      positivity
    have hc23 : (0:ℝ) ≤ (2 * A.C₁) ^ ((2:ℝ)/3) := Real.rpow_nonneg (by positivity) _
    calc ((yscale A.C₁ n : ℝ) + 1) * Real.log (zscale A.C₁ n : ℝ)
        ≤ ((2 * A.C₁) ^ ((2:ℝ)/3) * a ^ ((3:ℝ)/4) + 2) * (3 * b) := h2
      _ = 3 * (2 * A.C₁) ^ ((2:ℝ)/3) * (a ^ ((3:ℝ)/4) * b) + 6 * b := by ring
      _ ≤ 3 * (2 * A.C₁) ^ ((2:ℝ)/3) * a ^ ((7:ℝ)/8) + 6 * a ^ ((7:ℝ)/8) := by
          have := mul_le_mul_of_nonneg_left h34b (show (0:ℝ) ≤ 3 * (2 * A.C₁) ^ ((2:ℝ)/3) by positivity)
          linarith
      _ = (K / 100) * a ^ ((7:ℝ)/8) := by
          rw [hKdef]
          ring
      _ ≤ (a ^ ((1:ℝ)/8) / 100) * a ^ ((7:ℝ)/8) := by
          apply mul_le_mul_of_nonneg_right _ (by positivity)
          linarith
      _ = a / 100 := by
          rw [div_mul_eq_mul_div, mul_comm (a ^ ((1:ℝ)/8)) (a ^ ((7:ℝ)/8)), h78]
      _ ≤ 0.01 * a := by
          linarith
  -- second factor bound
  have hb_le_a : b ≤ a := by
    ...
    linarith
  have hTlogz : (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) ≤ 4 * a * (3 * b) :=
    mul_le_mul hT_up hlogz_up hlogz_pos.le (by linarith)
  have hone_plus : 1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) ≤ 13 * a ^ 2 := by
    nlinarith [hTlogz, mul_le_mul_of_nonneg_left hb_le_a ha0.le, ha50]
  have hfac_nn : (0:ℝ) ≤ 1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) := by
    have := mul_nonneg (Nat.cast_nonneg (Tscale n) : (0:ℝ) ≤ (Tscale n : ℝ)) hlogz_pos.le
    linarith
  have hzy_exp : (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) =
      Real.exp (((yscale A.C₁ n + 1 : ℕ) : ℝ) * Real.log (zscale A.C₁ n : ℝ)) := by
    rw [← Real.rpow_natCast (zscale A.C₁ n : ℝ) (yscale A.C₁ n + 1),
      Real.rpow_def_of_pos hzpos, mul_comm]
  have hzy_le : (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) ≤ Real.exp (0.01 * a) := by
    rw [hzy_exp]
    apply Real.exp_le_exp.mpr
    push_cast
    exact hkey
  have hP2 : (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
      (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) + 2 ≤ Real.exp (0.1 * a) := by
    have hfac_le : 1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) ≤ Real.exp (0.03 * a) :=
      le_trans hone_plus hsq
    have hmul : (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
        (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) ≤
  ...
  have hP1nn : (0:ℝ) ≤ Real.exp a / (1.5 * a * b) + 1 := by
    ...
    linarith
  have hP2nn : (0:ℝ) ≤ (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
      (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) + 2 := by
    have := mul_nonneg (pow_nonneg hzpos.le (yscale A.C₁ n + 1)) hfac_nn
    linarith
  calc (Real.exp a / (1.5 * a * b) + 1) *
      ((zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
        (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) + 2)
...
/-- Lemma 4.3 (Step 4 always finds a subset, and the pool suffices): from a
pool of `(log n)^{1.2}` primes one can extract a subset `S` whose product is
`≡ 1 (mod L)` and lands in `(n, n · x^{N*}]`. The paper's round-by-round
dynamic programming is the algorithmic realization; the statement is the
existence of the union of the extracted rounds. -/
theorem extraction :
    ∀ᶠ n : ℕ in atTop, ∀ Q ⊆ goodPrimes A.C₁ n, Q.card = Tscale n →
      ∀ k : ℕ, 0 < k → k.Coprime (Lmod Q) →
      (Real.log n) ^ (1.2 : ℝ) ≤ ((pool Q (zscale A.C₁ n) k).card : ℝ) →
      ∃ S : Finset ℕ, S ⊆ pool Q (zscale A.C₁ n) k ∧ S.Nonempty ∧
        (∏ p ∈ S, p) ≡ 1 [MOD Lmod Q] ∧
        n < ∏ p ∈ S, p ∧
  ...
  filter_upwards [eventually_ge_atTop 2,
    ...
    tendsto_ell3_atTop.eventually_ge_atTop 1,
    star_bound A] with n hn2 ha50 hb1 hstar
  ...
  have hCab : (1000:ℝ) * 50 * 1 ≤ A.C₁ * a * b :=
    mul_le_mul hCa hb1 (by norm_num) (le_trans (by norm_num) hCa)
  have hz_real : A.C₁ * a * b ≤ (zscale A.C₁ n : ℝ) := by
    rw [hzdef]
    exact Nat.le_ceil _
  have hz1 : 1 ≤ zscale A.C₁ n := by
    have h : (1:ℝ) ≤ (zscale A.C₁ n : ℝ) := le_trans (by linarith) hz_real
    exact_mod_cast h
  have hCb : (1000:ℝ) * 1 ≤ A.C₁ * b := mul_le_mul hC hb1 (by norm_num) (by linarith)
  have ha_le_Cab : a ≤ A.C₁ * a * b := by nlinarith [hCb, ha0]
  have hlogz_low : b ≤ Real.log (zscale A.C₁ n : ℝ) := by
  ...
  have hQfacts : ∀ q ∈ Q, q.Prime ∧ q ≤ zscale A.C₁ n ∧
      Real.sqrt (zscale A.C₁ n) < (q : ℝ) ∧ SmoothUpTo (yscale A.C₁ n) (q - 1) := by
    intro q hq
    have hmem := hQsub hq
    rw [goodPrimes, Finset.mem_filter, Finset.mem_range] at hmem
    have hz2 : 1 ≤ zscale A.C₁ n := by
      have := hmem.2.1.two_le
      have := hmem.1
      omega
    have h1z : (1 : ℝ) ≤ (zscale A.C₁ n : ℝ) := by exact_mod_cast hz2
    refine ⟨hmem.2.1, by omega, ?_, hmem.2.2.2⟩
    calc Real.sqrt (zscale A.C₁ n : ℝ)
        = (zscale A.C₁ n : ℝ) ^ ((1 : ℝ) / 2) := Real.sqrt_eq_rpow _
      _ ≤ (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) :=
  ...
  have hlogL_ge : 1.5 * a * b ≤ Real.log (Lmod Q) := by
    ...
    calc 1.5 * a * b = (3 * a) * (b / 2) := by ring
      _ ≤ (Tscale n : ℝ) * (Real.log (zscale A.C₁ n) / 2) := by
  ...
  -- λ and N* bounds
  have hlam_le : (lambdaL Q : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) := by
    have h := lambdaL_le_pow hz1
      (fun q hq => ⟨(hQfacts q hq).1, (hQfacts q hq).2.1, (hQfacts q hq).2.2.2⟩)
    exact_mod_cast h
  have hlogL_le : Real.log (Lmod Q) ≤ (Tscale n : ℝ) * Real.log (zscale A.C₁ n) := by
    have h := log_Lmod_le (fun q hq => (hQfacts q hq).2.1)
      (fun q hq => (hQprime q hq).one_lt.le)
    rwa [hQcard] at h
  have hNstar_le : (Nstar Q : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
      (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n)) + 2 := by
    ...
    have h1 : (lambdaL Q : ℝ) * (1 + Real.log (Lmod Q)) ≤
        (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
          (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n)) :=
  ...
  have hbudget : (Nat.log (Lmod Q + 1) n + 1) * Nstar Q ≤ (pool Q (zscale A.C₁ n) k).card := by
    have hP1nn : (0:ℝ) ≤ Real.exp a / (1.5 * a * b) + 1 := by
      have := div_nonneg (Real.exp_nonneg a) hcpos.le
      linarith
    have hcast : (((Nat.log (Lmod Q + 1) n + 1) * Nstar Q : ℕ) : ℝ) ≤
        ((pool Q (zscale A.C₁ n) k).card : ℝ) := by
      push_cast
      calc ((Nat.log (Lmod Q + 1) n : ℝ) + 1) * (Nstar Q : ℝ)
        ...
        _ ≤ ((pool Q (zscale A.C₁ n) k).card : ℝ) := hpool
    exact_mod_cast hcast
  -- one round of extraction is always possible
  have hQ_le_z : ∀ q ∈ Q, q ≤ zscale A.C₁ n ∧ q.Prime :=
    fun q hq => ⟨(hQfacts q hq).2.1, (hQfacts q hq).1⟩
  have hextract : ∀ W ⊆ pool Q (zscale A.C₁ n) k, W.card = Nstar Q →
  ...
  obtain ⟨S, hSsub, hScard, hSmod, hSdisj⟩ :=
    greedy_rounds (n := n) (L := Lmod Q) (Nst := Nstar Q) (x := xceil Q)
      (P := pool Q (zscale A.C₁ n) k) hn1 hL2 hx1
