/-
Windowed version of Lemma 4.2 of the paper (Step 3 halts), proved from an
...
which controls the reciprocal sum of the reservoir primes `q ∈ (w, z]`.

This is a transliteration of `Carmichael/Step3E.lean` to the scale window
`InWindow C₁ E n z w y T` of `Carmichael/DefsW.lean`: every scale fact that the
E proof read off a ceiling or floor is taken from the window instead
(`z ≤ 4 C₁ ℓ₂ ℓ₃`, `T ≤ 5 ℓ₂`, `z^{99/100} ≤ w + 1`), and the downstream
constants are re-derived. The smoothness component of `goodPrimesW` membership
is never used.
-/
...
/-- The window floor `(C₁ ℓ₂ ℓ₃)^{99/100}` tends to infinity. -/
private lemma tendsto_zrpowW (C₁ : ℝ) (h1000 : (1000 : ℝ) ≤ C₁) :
    Tendsto (fun n : ℕ => (C₁ * ell2 n * ell3 n) ^ ((99 : ℝ) / 100)) atTop atTop :=
  (tendsto_rpow_atTop (by norm_num : (0 : ℝ) < 99 / 100)).comp (tendsto_zreal C₁ h1000)
...
/-- Lemma 4.2 (Step 3 halts), windowed version: for `n` large, any scales in
the window, and any admissible `Q ⊆ goodPrimesW z w y`, some shift
`k ≤ x^{79/100}` coprime to `L` yields a pool of at least `c (log n)^{1.2}`
primes `dk + 1 ∈ (z, x]` with `d ∣ L`, for any prescribed constant `c`. -/
theorem step3_haltsW (C₁ D E : ℝ) (z₃ : ℕ) (c : ℝ)
    ...
    ∀ᶠ n : ℕ in Filter.atTop, ∀ z w y T : ℕ, InWindow C₁ E n z w y T →
      ...
      ∃ k : ℕ, 0 < k ∧ (k : ℝ) ≤ (xceil Q : ℝ) ^ ((79 : ℝ) / 100) ∧
        k.Coprime (Lmod Q) ∧
        c * (Real.log n) ^ (1.2 : ℝ) ≤ ((pool Q z k).card : ℝ) := by
  classical
  ...
  filter_upwards [eventually_ge_atTop 3,
    ...
    tendsto_ell3.eventually_ge_atTop (Real.log (4 * C₁)),
    tendsto_ell3.eventually_ge_atTop 11000,
    (tendsto_zrpowW C₁ h1000).eventually_ge_atTop 3,
    tendsto_ell2.eventually (eventually_quad_le_exp (75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2))),
    tendsto_ell2.eventually (eventually_quad_le_exp (5 * C₁))]
    with n hn3 ha50 haz₃ hb139 hbC hb11000 hmw3 hquadD hquadC
  intro z w y T hW Q hQsub hQcard
  obtain ⟨hz_lo, hz_hi, hw_lo, _hw_hi, _hy_lo, _hy_hi, hT_lo, hT_hi⟩ := hW
  ...
  have hzposR : (0 : ℝ) < (z : ℝ) := by linarith
  have hz1R : (1 : ℝ) < (z : ℝ) := by linarith
  have hzubR : (z : ℝ) ≤ 4 * (C₁ * a * b) := hz_hi
  -- log z bounds
  ...
  have hL1 : 1 < Lmod Q := by
    have h1 : 2 ^ 1 ≤ 2 ^ T := Nat.pow_le_pow_right (by norm_num) hT1
    omega
  ...
  have hL2TR : (2 : ℝ) ^ T ≤ (Lmod Q : ℝ) := by exact_mod_cast hL2T
  -- x facts
  ...
  have hlogx_pos : (0 : ℝ) < Real.log (xceil Q : ℝ) := by
    ...
    linarith
  have hlogL_ub : Real.log (Lmod Q : ℝ) ≤ (T : ℝ) * Real.log (z : ℝ) := by
    have h1 : Real.log (Lmod Q : ℝ) ≤ Real.log ((z : ℝ) ^ T) := by
      apply Real.log_le_log hL0R
      exact_mod_cast hLzT
    rwa [Real.log_pow] at h1
  have hlogx_ub : Real.log (xceil Q : ℝ) ≤ 75 * (a * b) := by
    have hlogx_eq : Real.log (xceil Q : ℝ) = 5 * Real.log (Lmod Q : ℝ) := by
      rw [hxR, Real.log_pow]
      push_cast
      ring
    rw [hlogx_eq]
    have h1 : (T : ℝ) * Real.log (z : ℝ) ≤ 5 * a * (3 * b) :=
      mul_le_mul hT_ub hlogz_ub hlogz_pos.le (by linarith)
    linarith [hlogL_ub]
  -- the Mertens window (w, z], with w ≥ z^{99/100} - 1 from the window
  have hrw_pos : (0 : ℝ) < (z : ℝ) ^ ((99 : ℝ) / 100) :=
    Real.rpow_pos_of_pos hzposR _
  have hrw3 : (3 : ℝ) ≤ (z : ℝ) ^ ((99 : ℝ) / 100) :=
    le_trans hmw3 (Real.rpow_le_rpow (by linarith [hCab]) hz_lo (by norm_num))
  have hw2R : (2 : ℝ) ≤ (w : ℝ) := by linarith [hw_lo, hrw3]
  have hw2 : 2 ≤ w := by exact_mod_cast hw2R
  have hw_half : (z : ℝ) ^ ((99 : ℝ) / 100) / 2 ≤ (w : ℝ) := by
    linarith [hw_lo, hrw3]
  ...
  have hwz : w ≤ z := by
    obtain ⟨q, hq⟩ := hQne
    obtain ⟨_, hqz, hwq⟩ := hQfacts q hq
    omega
  have hlogw_lb : 197 / 200 * Real.log (z : ℝ) ≤ Real.log (w : ℝ) := by
    have h1 : Real.log ((z : ℝ) ^ ((99 : ℝ) / 100) / 2) ≤ Real.log (w : ℝ) :=
  ...
  have hsum_Q : ∑ q ∈ Q, (1 : ℝ) / q ≤ 3 / 160 := by
    ...
    have hQsub2 : Q ⊆ ((Finset.range (z + 1)).filter Nat.Prime) \
        ((Finset.range (w + 1)).filter Nat.Prime) := by
      intro q hq
      obtain ⟨hqp, hqz, hwq⟩ := hQfacts q hq
    ...
    have h2 : ∑ p ∈ ((Finset.range (z + 1)).filter Nat.Prime) \
        ((Finset.range (w + 1)).filter Nat.Prime), (1 : ℝ) / p
        = primeRecipSum z - primeRecipSum w := by
      simp only [primeRecipSum]
      exact Finset.sum_sdiff_eq_sub hsub_ranges
    rw [h2] at h1
    have h3 : primeRecipSum z - primeRecipSum w ≤
        (Real.log (z : ℝ) - Real.log (w : ℝ) + 2 * (Real.log 4 + 4)) /
          Real.log (w : ℝ) :=
    ...
    have hlog4 : Real.log 4 < 1.39 := by
      ...
      push_cast
      linarith
    have hZ11000 : (11000 : ℝ) ≤ Real.log (z : ℝ) :=
      le_trans hb11000 hlogz_lb
    have h4 : (Real.log (z : ℝ) - Real.log (w : ℝ) + 2 * (Real.log 4 + 4)) /
  ...
  have hz₃x : z₃ < xceil Q := by
    have h1 : z₃ ≤ T := by
      have h : (z₃ : ℝ) ≤ (T : ℝ) := by linarith [haz₃, ha0, hT_lb]
      exact_mod_cast h
    have h2 : T < 2 ^ T := Nat.lt_two_pow_self
  ...
  have hxr310 : (xceil Q : ℝ) ^ ((3 : ℝ) / 10) = (Lmod Q : ℝ) ^ ((3 : ℝ) / 2) := by
    ...
    norm_num
  have h2Texp : ((2 : ℝ) ^ T) ^ ((3 : ℝ) / 2)
      = Real.exp (Real.log 2 * ((T : ℝ) * (3 / 2))) := by
    rw [← Real.rpow_natCast (2 : ℝ) T, ← Real.rpow_mul (by norm_num : (0 : ℝ) ≤ 2),
  ...
  have hqbound310 : ∀ q : ℕ, q.Prime → q ∣ Lmod Q →
    ...
    have hq_le_z : (q : ℝ) ≤ (z : ℝ) := by
      exact_mod_cast (hQfacts q hqQ).2.1
    calc (q : ℝ) ≤ (z : ℝ) := hq_le_z
      _ ≤ Real.exp (Real.log 2 * ((T : ℝ) * (3 / 2))) := hzle_pow
      _ = ((2 : ℝ) ^ T) ^ ((3 : ℝ) / 2) := h2Texp.symm
      _ ≤ (Lmod Q : ℝ) ^ ((3 : ℝ) / 2) :=
  ...
  have hdivcard : (Lmod Q).divisors.card = 2 ^ T := by
  ...
  have hpool_card : (pool Q z k).card
  ...
  have hsdiff_sub : ((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
    ...
    have h1 : ¬ (z < d * k + 1) := fun hzlt => hnot ⟨hdd, hdx, hdp, hzlt⟩
    have h2 : d * k + 1 ≤ z := Nat.not_lt.mp h1
  ...
  have hcard_split : ((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime)).card ≤
      ((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime ∧ z < d * k + 1)).card + (z + 1) := by
  ...
  have h2T_ge : Real.exp (2.07 * a) ≤ (2 : ℝ) ^ T := by
    have h1 : (2 : ℝ) ^ T = Real.exp (Real.log 2 * (T : ℝ)) := by
      rw [← Real.rpow_natCast (2 : ℝ) T, Real.rpow_def_of_pos (by norm_num)]
    rw [h1]
    apply Real.exp_le_exp.mpr
    have h6 : (0.6931471803 : ℝ) * (3 * a) ≤ Real.log 2 * (T : ℝ) :=
      mul_le_mul hlog2_lb.le hT_lb (by linarith [ha0]) (le_trans (by norm_num) hlog2_lb.le)
    linarith [ha0]
  have hab_pos : (0 : ℝ) < a * b := mul_pos ha0 hb0
  have hquad87 : 75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * (a * b) ≤ Real.exp (0.87 * a) := by
    have hK0 : (0 : ℝ) ≤ 75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) := by positivity
    have h1 : 75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * (a * b)
        ≤ 75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * a ^ 2 := by
      apply mul_le_mul_of_nonneg_left _ hK0
      nlinarith [hb_le_a, ha0]
    calc 75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * (a * b)
        ≤ 75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * a ^ 2 := h1
      _ ≤ Real.exp (0.8 * a) := hquadD
      _ ≤ Real.exp (0.87 * a) := Real.exp_le_exp.mpr (by linarith [ha0])
  have hzp1_exp : (z : ℝ) + 1 ≤ Real.exp (1.2 * a) := by
    ...
    have h1 : (z : ℝ) + 1 ≤ 5 * C₁ * a ^ 2 := by
      nlinarith [hzubR, h1a, h1b]
    calc (z : ℝ) + 1 ≤ 5 * C₁ * a ^ 2 := h1
  ...
  have hmain : c * Real.exp (1.2 * a) + ((z : ℝ) + 1)
      ≤ (2 : ℝ) ^ (-D - 2) / Real.log (xceil Q : ℝ) * ((2 : ℝ) ^ T) := by
    have h75ab_pos : (0 : ℝ) < 75 * (a * b) := by linarith [hab_pos]
    have h1 : (2 : ℝ) ^ (-D - 2) / (75 * (a * b))
        ≤ (2 : ℝ) ^ (-D - 2) / Real.log (xceil Q : ℝ) :=
      div_le_div_of_nonneg_left hc₀pos.le hlogx_pos hlogx_ub
    have h2 : (2 : ℝ) ^ (-D - 2) / (75 * (a * b)) * Real.exp (2.07 * a)
        ≤ (2 : ℝ) ^ (-D - 2) / Real.log (xceil Q : ℝ) * ((2 : ℝ) ^ T) :=
      mul_le_mul h1 h2T_ge (Real.exp_nonneg _) hc₀logx_nonneg
    have h4 : Real.exp (2.07 * a) = Real.exp (0.87 * a) * Real.exp (1.2 * a) := by
      rw [← Real.exp_add]
      congr 1
      ring
    have h5 : |c| + 1 ≤ (2 : ℝ) ^ (-D - 2) / (75 * (a * b)) * Real.exp (0.87 * a) := by
      have hc₀ne : (2 : ℝ) ^ (-D - 2) ≠ 0 := ne_of_gt hc₀pos
      have habne : a * b ≠ 0 := ne_of_gt hab_pos
      have h6 : (2 : ℝ) ^ (-D - 2) / (75 * (a * b))
          * (75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * (a * b)) = |c| + 1 := by
        field_simp
      have h8 : (2 : ℝ) ^ (-D - 2) / (75 * (a * b))
          * (75 * (|c| + 1) / (2 : ℝ) ^ (-D - 2) * (a * b))
          ≤ (2 : ℝ) ^ (-D - 2) / (75 * (a * b)) * Real.exp (0.87 * a) :=
        mul_le_mul_of_nonneg_left hquad87 (div_nonneg hc₀pos.le h75ab_pos.le)
      linarith [h6, h8]
    have h7 : (|c| + 1) * Real.exp (1.2 * a)
        ≤ (2 : ℝ) ^ (-D - 2) / (75 * (a * b)) * Real.exp (2.07 * a) := by
      rw [h4, ← mul_assoc]
      exact mul_le_mul_of_nonneg_right h5 (Real.exp_nonneg _)
    have h9 : c * Real.exp (1.2 * a) ≤ |c| * Real.exp (1.2 * a) :=
      mul_le_mul_of_nonneg_right (le_abs_self c) (Real.exp_nonneg _)
    linarith [hzp1_exp, h2, h7, h9]
  -- conclude
  rw [hgoal_exp, hpool_card]
  have hcastT : ((2 ^ T : ℕ) : ℝ) = (2 : ℝ) ^ T := by push_cast; ring
  rw [hcastT] at hcount
  have hcast_split : (((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime)).card : ℝ) ≤
      (((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime ∧ z < d * k + 1)).card : ℝ)
      + ((z : ℝ) + 1) := by
