/-
Lemma 4.2 of the paper (Step 3 halts). Proved from `A.pigeonhole`
(AGP Theorem 3.1 at B = 2/5) together with the Mertens-type bound
`primeRecipSum_sub_le` (proved in `Carmichael/RecipSum.lean`), which
controls the reciprocal sum of the reservoir primes `q ∈ (z^{99/100}, z]`.
-/
...
private lemma tendsto_zreal :
    Tendsto (fun n : ℕ => A.C₁ * ell2 n * ell3 n) atTop atTop := by
  ...
  have hC : (0 : ℝ) < A.C₁ := A.C₁_pos
  have hC1000 : (1000 : ℝ) ≤ A.C₁ := A.thousand_le_C₁
  have hCℓ₂ : A.C₁ * 1 ≤ A.C₁ * ell2 n := mul_le_mul_of_nonneg_left h2 hC.le
  have h1 : (1 : ℝ) ≤ A.C₁ * ell2 n := by linarith
  have := mul_le_mul_of_nonneg_right h1 h3
  linarith

private lemma tendsto_zcast :
    Tendsto (fun n : ℕ => (zscale A.C₁ n : ℝ)) atTop atTop :=
  tendsto_atTop_mono (fun _ => Nat.le_ceil _) (tendsto_zreal A)

private lemma tendsto_zrpow :
    Tendsto (fun n : ℕ => (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100)) atTop atTop :=
  (tendsto_rpow_atTop (by norm_num : (0 : ℝ) < 99 / 100)).comp (tendsto_zcast A)
...
/-- Lemma 4.2 (Step 3 halts): for `n` large and any admissible `Q`, some
shift `k ≤ x^{3/5}` coprime to `L` yields a pool of at least
`(log n)^{1.2}` primes `dk + 1 ∈ (z, x]` with `d ∣ L`. -/
theorem step3_halts :
    ∀ᶠ n : ℕ in atTop, ∀ Q ⊆ goodPrimes A.C₁ n, Q.card = Tscale n →
      ∃ k : ℕ, 0 < k ∧ (k : ℝ) ≤ (xceil Q : ℝ) ^ ((3 : ℝ) / 5) ∧
        k.Coprime (Lmod Q) ∧
        (Real.log n) ^ (1.2 : ℝ) ≤ ((pool Q (zscale A.C₁ n) k).card : ℝ) := by
  classical
  have hC : (1000 : ℝ) ≤ A.C₁ := A.thousand_le_C₁
  have hc₀pos : (0 : ℝ) < (2 : ℝ) ^ (-A.D - 2) := Real.rpow_pos_of_pos (by norm_num) _
  filter_upwards [eventually_ge_atTop 3,
    tendsto_ell2.eventually_ge_atTop 50,
    tendsto_ell2.eventually_ge_atTop (A.z₃ : ℝ),
    tendsto_ell3.eventually_ge_atTop 139,
    tendsto_ell3.eventually_ge_atTop (Real.log (2 * A.C₁)),
    tendsto_ell3.eventually_ge_atTop 11000,
    (tendsto_zrpow A).eventually_ge_atTop 3,
    tendsto_ell2.eventually (eventually_quad_le_exp (120 / (2 : ℝ) ^ (-A.D - 2))),
    tendsto_ell2.eventually (eventually_quad_le_exp (3 * A.C₁))]
  ...
  have hb_le_a : b ≤ a := by
    ...
    linarith
  -- z bounds
  have hzlbR : A.C₁ * a * b ≤ (zscale A.C₁ n : ℝ) := by
    rw [hzdef]; exact Nat.le_ceil _
  have hCa : (1000 : ℝ) * 50 ≤ A.C₁ * a := mul_le_mul hC ha50 (by norm_num) (by linarith)
  have hCab : (1000 : ℝ) * 50 * 139 ≤ A.C₁ * a * b :=
    mul_le_mul hCa hb139 (by norm_num) (le_trans (by norm_num) hCa)
  have hzposR : (0 : ℝ) < (zscale A.C₁ n : ℝ) := by linarith
  have hz1R : (1 : ℝ) < (zscale A.C₁ n : ℝ) := by linarith
  have hzubR : (zscale A.C₁ n : ℝ) ≤ 2 * (A.C₁ * a * b) := by
    rw [hzdef]
    have h := Nat.ceil_lt_add_one (show (0 : ℝ) ≤ A.C₁ * a * b by linarith)
    linarith
  -- log z bounds
  have hCb : (1000 : ℝ) * 139 ≤ A.C₁ * b := mul_le_mul hC hb139 (by norm_num) (by linarith)
  have ha_le_Cab : a ≤ A.C₁ * a * b := by nlinarith [mul_le_mul_of_nonneg_left hCb ha0.le]
  have hlogz_lb : b ≤ Real.log (zscale A.C₁ n : ℝ) := by
    rw [hba]
    exact Real.log_le_log ha0 (le_trans ha_le_Cab hzlbR)
  have hlogz_pos : (0 : ℝ) < Real.log (zscale A.C₁ n : ℝ) := by linarith
  have hlogb_le : Real.log b ≤ b := by
    have := Real.log_le_sub_one_of_pos hb0
    linarith
  have h2C₁pos : (0 : ℝ) < 2 * A.C₁ := by linarith
  have hlogz_ub : Real.log (zscale A.C₁ n : ℝ) ≤ 3 * b := by
    calc Real.log (zscale A.C₁ n : ℝ) ≤ Real.log (2 * A.C₁ * (a * b)) := by
          apply Real.log_le_log hzposR
          calc (zscale A.C₁ n : ℝ) ≤ 2 * (A.C₁ * a * b) := hzubR
            _ = 2 * A.C₁ * (a * b) := by ring
  ...
  have hQfacts : ∀ q ∈ Q, q.Prime ∧ q ≤ zscale A.C₁ n ∧
      (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) < (q : ℝ) := by
    intro q hq
    have hmem := hQsub hq
    simp only [goodPrimes, Finset.mem_filter, Finset.mem_range] at hmem
  ...
  have hLzT : Lmod Q ≤ zscale A.C₁ n ^ Tscale n := by
    rw [← hQcard]
  ...
  have hlogx_pos : (0 : ℝ) < Real.log (xceil Q : ℝ) := by
    ...
    linarith
  have hlogL_ub : Real.log (Lmod Q : ℝ) ≤ (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) := by
    have h1 : Real.log (Lmod Q : ℝ) ≤ Real.log ((zscale A.C₁ n : ℝ) ^ Tscale n) := by
  ...
  have hlogx_ub : Real.log (xceil Q : ℝ) ≤ 60 * (a * b) := by
    have hlogx_eq : Real.log (xceil Q : ℝ) = 5 * Real.log (Lmod Q : ℝ) := by
      ...
      ring
    rw [hlogx_eq]
    have h1 : (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) ≤ 4 * a * (3 * b) :=
      mul_le_mul hT_ub hlogz_ub hlogz_pos.le (by linarith)
    linarith [hlogL_ub]
  -- the Mertens window (z^{99/100}, z]
  set w := ⌊(zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100)⌋₊ with hwdef
  have hrw_pos : (0 : ℝ) < (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) :=
    Real.rpow_pos_of_pos hzposR _
  have hrw2 : (2 : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) := by
    linarith [hmw3]
  have hwleR : (w : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) := Nat.floor_le hrw_pos.le
  have hw2 : 2 ≤ w := Nat.le_floor (by exact_mod_cast hrw2)
  have hw_half : (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) / 2 ≤ (w : ℝ) := by
    have h1 := Nat.lt_floor_add_one ((zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100))
    linarith
  have hz99_le_z : (zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) ≤ (zscale A.C₁ n : ℝ) := by
    have h := Real.rpow_le_rpow_of_exponent_le hz1R.le
      (show (99 : ℝ) / 100 ≤ 1 by norm_num)
    rwa [Real.rpow_one] at h
  have hwz : w ≤ zscale A.C₁ n := by
    have h : (w : ℝ) ≤ (zscale A.C₁ n : ℝ) := le_trans hwleR hz99_le_z
    exact_mod_cast h
  have hlogw_lb : 197 / 200 * Real.log (zscale A.C₁ n : ℝ) ≤ Real.log (w : ℝ) := by
    have h1 : Real.log ((zscale A.C₁ n : ℝ) ^ ((99 : ℝ) / 100) / 2) ≤ Real.log (w : ℝ) :=
  ...
  have hsum_Q : ∑ q ∈ Q, (1 : ℝ) / q ≤ 3 / 160 := by
    have hsub_ranges : (Finset.range (w + 1)).filter Nat.Prime ⊆
        (Finset.range (zscale A.C₁ n + 1)).filter Nat.Prime :=
      Finset.filter_subset_filter _ (Finset.range_subset_range.mpr (Nat.succ_le_succ hwz))
    have hQsub2 : Q ⊆ ((Finset.range (zscale A.C₁ n + 1)).filter Nat.Prime) \
      ...
      linarith [hwleR, hq99]
    have h1 : ∑ q ∈ Q, (1 : ℝ) / q ≤
        ∑ p ∈ ((Finset.range (zscale A.C₁ n + 1)).filter Nat.Prime) \
          ((Finset.range (w + 1)).filter Nat.Prime), (1 : ℝ) / p :=
      Finset.sum_le_sum_of_subset_of_nonneg hQsub2 (fun i _ _ => by positivity)
    have h2 : ∑ p ∈ ((Finset.range (zscale A.C₁ n + 1)).filter Nat.Prime) \
        ((Finset.range (w + 1)).filter Nat.Prime), (1 : ℝ) / p
        = primeRecipSum (zscale A.C₁ n) - primeRecipSum w := by
      simp only [primeRecipSum]
      exact Finset.sum_sdiff_eq_sub hsub_ranges
    rw [h2] at h1
    have h3 : primeRecipSum (zscale A.C₁ n) - primeRecipSum w ≤
        (Real.log (zscale A.C₁ n : ℝ) - Real.log (w : ℝ) + 2 * (Real.log 4 + 4)) /
          Real.log (w : ℝ) :=
    ...
    have hlog4 : Real.log 4 < 1.39 := by
      ...
      push_cast
      linarith
    have hZ11000 : (11000 : ℝ) ≤ Real.log (zscale A.C₁ n : ℝ) :=
      le_trans hb11000 hlogz_lb
    have h4 : (Real.log (zscale A.C₁ n : ℝ) - Real.log (w : ℝ) + 2 * (Real.log 4 + 4)) /
        Real.log (w : ℝ) ≤ 3 / 160 := by
      rw [div_le_iff₀ hlogw_pos]
      linarith [hlogw_lb]
    linarith [h1, h3, h4]
  -- x exceeds the AGP threshold
  have hz₃x : A.z₃ < xceil Q := by
    have h1 : A.z₃ ≤ Tscale n := by
      have h : (A.z₃ : ℝ) ≤ (Tscale n : ℝ) := by linarith [haz₃, ha0, hT_lb]
      exact_mod_cast h
  ...
  have hqbound : ∀ q : ℕ, q.Prime → q ∣ Lmod Q →
    ...
    have hq_le_z : (q : ℝ) ≤ (zscale A.C₁ n : ℝ) := by
      exact_mod_cast (hQfacts q hqQ).2.1
    calc (q : ℝ) ≤ (zscale A.C₁ n : ℝ) := hq_le_z
  ...
  obtain ⟨k, hk0, hkx, hkL, hcount⟩ :=
    A.pigeonhole (xceil Q) (Lmod Q) hz₃x hL1 hsqL hqbound hsumL
  ...
  have hsdiff_sub : ((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      ...
      (d * k + 1).Prime ∧ zscale A.C₁ n < d * k + 1)) ⊆
      Finset.range (zscale A.C₁ n + 1) := by
    intro d hd
    rw [Finset.mem_sdiff, Finset.mem_filter, Finset.mem_filter] at hd
    obtain ⟨⟨hdd, hdx, hdp⟩, hnot⟩ := hd
    have h1 : ¬ (zscale A.C₁ n < d * k + 1) := fun hzlt => hnot ⟨hdd, hdx, hdp, hzlt⟩
    have h2 : d * k + 1 ≤ zscale A.C₁ n := Nat.not_lt.mp h1
  ...
  have hcard_split : ((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime)).card ≤
      ((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime ∧ zscale A.C₁ n < d * k + 1)).card + (zscale A.C₁ n + 1) := by
  ...
  have hquad87 : 120 / (2 : ℝ) ^ (-A.D - 2) * (a * b) ≤ Real.exp (0.87 * a) := by
    have hK0 : (0 : ℝ) ≤ 120 / (2 : ℝ) ^ (-A.D - 2) := by positivity
    have h1 : 120 / (2 : ℝ) ^ (-A.D - 2) * (a * b)
        ≤ 120 / (2 : ℝ) ^ (-A.D - 2) * a ^ 2 := by
      apply mul_le_mul_of_nonneg_left _ hK0
      nlinarith [hb_le_a, ha0]
    calc 120 / (2 : ℝ) ^ (-A.D - 2) * (a * b)
        ≤ 120 / (2 : ℝ) ^ (-A.D - 2) * a ^ 2 := h1
      _ ≤ Real.exp (0.8 * a) := hquadD
      _ ≤ Real.exp (0.87 * a) := Real.exp_le_exp.mpr (by linarith [ha0])
  have hzp1_exp : (zscale A.C₁ n : ℝ) + 1 ≤ Real.exp (1.2 * a) := by
    have hC₁a_pos : (0 : ℝ) < A.C₁ * a := mul_pos (by linarith) ha0
    have h1a : A.C₁ * a * b ≤ A.C₁ * a * a :=
      mul_le_mul_of_nonneg_left hb_le_a hC₁a_pos.le
    have h1b : (1 : ℝ) ≤ A.C₁ * a * a := by nlinarith [hC, ha50]
    have h1 : (zscale A.C₁ n : ℝ) + 1 ≤ 3 * A.C₁ * a ^ 2 := by
      nlinarith [hzubR, h1a, h1b]
    calc (zscale A.C₁ n : ℝ) + 1 ≤ 3 * A.C₁ * a ^ 2 := h1
      _ ≤ Real.exp (0.8 * a) := hquadC
      _ ≤ Real.exp (1.2 * a) := Real.exp_le_exp.mpr (by linarith [ha0])
  have hc₀logx_nonneg : (0 : ℝ) ≤ (2 : ℝ) ^ (-A.D - 2) / Real.log (xceil Q : ℝ) :=
    div_nonneg hc₀pos.le hlogx_pos.le
  have hmain : Real.exp (1.2 * a) + ((zscale A.C₁ n : ℝ) + 1)
      ≤ (2 : ℝ) ^ (-A.D - 2) / Real.log (xceil Q : ℝ) * ((2 : ℝ) ^ Tscale n) := by
    have h60ab_pos : (0 : ℝ) < 60 * (a * b) := by linarith [hab_pos]
    have h1 : (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b))
        ≤ (2 : ℝ) ^ (-A.D - 2) / Real.log (xceil Q : ℝ) :=
      div_le_div_of_nonneg_left hc₀pos.le hlogx_pos hlogx_ub
    have h2 : (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b)) * Real.exp (2.07 * a)
        ≤ (2 : ℝ) ^ (-A.D - 2) / Real.log (xceil Q : ℝ) * ((2 : ℝ) ^ Tscale n) :=
      mul_le_mul h1 h2T_ge (Real.exp_nonneg _) hc₀logx_nonneg
    have h4 : Real.exp (2.07 * a) = Real.exp (0.87 * a) * Real.exp (1.2 * a) := by
      rw [← Real.exp_add]
      congr 1
      ring
    have h5 : (2 : ℝ) ≤ (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b)) * Real.exp (0.87 * a) := by
      have hc₀ne : (2 : ℝ) ^ (-A.D - 2) ≠ 0 := ne_of_gt hc₀pos
      have habne : a * b ≠ 0 := ne_of_gt hab_pos
      have h6 : (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b))
          * (120 / (2 : ℝ) ^ (-A.D - 2) * (a * b)) = 2 := by
        field_simp
        norm_num
      have h8 : (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b))
          * (120 / (2 : ℝ) ^ (-A.D - 2) * (a * b))
          ≤ (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b)) * Real.exp (0.87 * a) :=
        mul_le_mul_of_nonneg_left hquad87 (div_nonneg hc₀pos.le h60ab_pos.le)
      linarith [h6, h8]
    have h7 : 2 * Real.exp (1.2 * a)
        ≤ (2 : ℝ) ^ (-A.D - 2) / (60 * (a * b)) * Real.exp (2.07 * a) := by
      rw [h4, ← mul_assoc]
  ...
  have hcast_split : (((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime)).card : ℝ) ≤
      (((Lmod Q).divisors.filter (fun d => d * k + 1 ≤ xceil Q ∧
      (d * k + 1).Prime ∧ zscale A.C₁ n < d * k + 1)).card : ℝ)
      + ((zscale A.C₁ n : ℝ) + 1) := by
