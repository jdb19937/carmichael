/-
Lemma 4.4 of the paper (the output is Carmichael, in (n, n^{1+ε}]). Proved
from Korselt's criterion (`Carmichael/Korselt.lean`) and divisibility
bookkeeping.
-/
...
/-- Unpack membership in the reservoir of good primes. -/
private lemma mem_goodPrimes {C₁ : ℝ} {n q : ℕ} (hq : q ∈ goodPrimes C₁ n) :
    q.Prime ∧ q ≤ zscale C₁ n ∧ SmoothUpTo (yscale C₁ n) (q - 1) := by
  simp only [goodPrimes, Finset.mem_filter, Finset.mem_range] at hq
...
phrased in terms of the single variable `u = ℓ₂(n) → ∞`. -/

private lemma eventually_u (ε C₁ : ℝ) (hε : 0 < ε) (hC₁ : 1 ≤ C₁) :
    ∀ᶠ u : ℝ in atTop,
      1 ≤ ⌈C₁ * u * Real.log u⌉₊ ∧
      (⌈C₁ * u * Real.log u⌉₊ : ℝ) ^ (10 * ⌈(3 : ℝ) * u⌉₊) ≤ Real.exp (Real.exp u) ∧
      ((⌈C₁ * u * Real.log u⌉₊ : ℝ) ^ (⌈(⌈C₁ * u * Real.log u⌉₊ : ℝ) ^ ((2 : ℝ) / 3)⌉₊ + 1) *
  ...
  · -- (z : ℝ) ^ (10 T) ≤ n
    ...
    linarith [hexpu]
  · -- the overshoot bound
    have hX0 : (0 : ℝ) ≤ (2 * C₁ * u * v) ^ ((2 : ℝ) / 3) :=
      Real.rpow_nonneg (by linarith) _
    have hW0 : (0 : ℝ) ≤ ((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v) :=
      mul_nonneg (by linarith) (by linarith)
    -- E = Z^(yc+1) ≤ exp W
    have hyub : (yc : ℝ) + 1 ≤ (2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2 := by
      have h1 : (yc : ℝ) < Z ^ ((2 : ℝ) / 3) + 1 :=
        Nat.ceil_lt_add_one (Real.rpow_nonneg hZ0.le _)
      have h2 : Z ^ ((2 : ℝ) / 3) ≤ (2 * C₁ * u * v) ^ ((2 : ℝ) / 3) :=
        Real.rpow_le_rpow hZ0.le hZub (by norm_num)
      linarith
    have hE : Z ^ (yc + 1)
        ≤ Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v)) := by
      have heq : Z ^ (yc + 1) = Real.exp (((yc + 1 : ℕ) : ℝ) * Real.log Z) := by
        rw [← Real.log_pow, Real.exp_log (pow_pos hZ0 _)]
      rw [heq, show ((yc + 1 : ℕ) : ℝ) = (yc : ℝ) + 1 by push_cast; ring]
      exact Real.exp_le_exp.mpr (mul_le_mul hyub hlZ hlZ0 (by linarith))
    have hexpW1 : (1 : ℝ) ≤ Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v)) := by
      rw [show (1 : ℝ) = Real.exp 0 from Real.exp_zero.symm]
      exact Real.exp_le_exp.mpr hW0
    have h1 : Tc * Real.log Z ≤ 4 * u * (3 * v) := mul_le_mul hTub hlZ hlZ0 (by linarith)
    ...
    have hEP : Z ^ (yc + 1) * (1 + Tc * Real.log Z) + 2
        ≤ 15 * (Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v)) * (u * v)) := by
      have h3 : Z ^ (yc + 1) * (1 + Tc * Real.log Z)
          ≤ Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v)) * (13 * (u * v)) :=
        mul_le_mul hE hP hP0 (Real.exp_pos _).le
      have h4 : (1 : ℝ)
          ≤ Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v)) * (u * v) := by
        calc (1 : ℝ) = 1 * 1 := by ring
          _ ≤ _ := mul_le_mul hexpW1 huv1 (by norm_num) (Real.exp_pos _).le
      linarith [h3, h4]
    have hEP0 : (0 : ℝ)
        ≤ 15 * (Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v)) * (u * v)) :=
    ...
    have hF1 : 3 * ((2 * C₁ * u * v) ^ ((2 : ℝ) / 3)) * v ≤ u / 2 := by
      have hXeq : (2 * C₁ * u * v) ^ ((2 : ℝ) / 3)
          = (2 * C₁) ^ ((2 : ℝ) / 3) * (u ^ ((2 : ℝ) / 3) * v ^ ((2 : ℝ) / 3)) := by
        rw [show 2 * C₁ * u * v = 2 * C₁ * (u * v) by ring,
          Real.mul_rpow (by linarith) huv0, Real.mul_rpow hu0.le hv0.le]
      have hv53 : v ^ ((2 : ℝ) / 3) * v = v ^ ((5 : ℝ) / 3) := by
        rw [← Real.rpow_add_one hv0.ne' ((2 : ℝ) / 3)]
        norm_num
      have hv536 : v ^ ((5 : ℝ) / 3) ≤ u ^ ((5 : ℝ) / 36) := by
        calc v ^ ((5 : ℝ) / 3) ≤ (u ^ ((1 : ℝ) / 12)) ^ ((5 : ℝ) / 3) :=
              Real.rpow_le_rpow hv0.le hlogu (by norm_num)
          _ = u ^ ((5 : ℝ) / 36) := by
              rw [← Real.rpow_mul hu0.le]
              norm_num
      have e1 : 3 * ((2 * C₁ * u * v) ^ ((2 : ℝ) / 3)) * v
          = 3 * (2 * C₁) ^ ((2 : ℝ) / 3) * (u ^ ((2 : ℝ) / 3) * (v ^ ((2 : ℝ) / 3) * v)) := by
        rw [hXeq]; ring
      rw [e1, hv53]
      have e2 : u ^ ((2 : ℝ) / 3) * v ^ ((5 : ℝ) / 3) ≤ u ^ ((29 : ℝ) / 36) := by
        calc u ^ ((2 : ℝ) / 3) * v ^ ((5 : ℝ) / 3)
            ≤ u ^ ((2 : ℝ) / 3) * u ^ ((5 : ℝ) / 36) :=
              mul_le_mul_of_nonneg_left hv536 (Real.rpow_nonneg hu0.le _)
          _ = u ^ ((29 : ℝ) / 36) := by
              rw [← Real.rpow_add hu0]
              norm_num
      have e3 : (0 : ℝ) ≤ 3 * (2 * C₁) ^ ((2 : ℝ) / 3) := by
        have := Real.rpow_nonneg (show (0 : ℝ) ≤ 2 * C₁ by linarith) ((2 : ℝ) / 3)
        linarith
      calc 3 * (2 * C₁) ^ ((2 : ℝ) / 3) * (u ^ ((2 : ℝ) / 3) * v ^ ((5 : ℝ) / 3))
          ≤ 3 * (2 * C₁) ^ ((2 : ℝ) / 3) * u ^ ((29 : ℝ) / 36) :=
            mul_le_mul_of_nonneg_left e2 e3
        _ ≤ (u ^ ((7 : ℝ) / 36) / 2) * u ^ ((29 : ℝ) / 36) := by
            refine mul_le_mul_of_nonneg_right ?_ (Real.rpow_nonneg hu0.le _)
            linarith [hK]
        _ = u / 2 := by
            rw [div_mul_eq_mul_div, ← Real.rpow_add hu0,
              show (7 : ℝ) / 36 + 29 / 36 = 1 by norm_num, Real.rpow_one]
    ...
    calc (Z ^ (yc + 1) * (1 + Tc * Real.log Z) + 2) * (5 * Tc * Real.log Z)
      ...
      _ ≤ 900 * (Real.exp (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v))
          ...
          have h6 := mul_le_mul_of_nonneg_left h5 (Real.exp_pos
            (((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v))).le
          linarith
      _ = Real.exp ((((2 * C₁ * u * v) ^ ((2 : ℝ) / 3) + 2) * (3 * v))
            + (Real.log 900 + 4 * v)) := by
          rw [Real.exp_add, Real.exp_add, hexp4v,
...
/-- Lemma 4.4 (the output is Carmichael, in `(n, n^{1+ε}]`): any subset of
the pool as produced by `extraction` is a Carmichael number with at least
three prime factors, and the overshoot `x^{N*}` is at most `n^ε` for `n`
large. Uses Korselt's criterion. -/
theorem output_carmichael (ε : ℝ) (hε : 0 < ε) :
  ...
  have hC₁ : (1 : ℝ) ≤ A.C₁ := le_trans (by norm_num) A.thousand_le_C₁
  ...
  filter_upwards [htend.eventually (eventually_u ε A.C₁ hε hC₁), eventually_ge_atTop 2]
    with n hpack hn2
  obtain ⟨hz1c, hP2c, hP3c⟩ := hpack
  -- translate the eventual facts to the scale functions (all `rfl`-conversions)
  have hz1' : 1 ≤ zscale A.C₁ n := hz1c
  have hP2' : ((zscale A.C₁ n : ℝ)) ^ (10 * Tscale n) ≤ Real.exp (Real.exp (ell2 n)) := hP2c
  have hP3' : (((zscale A.C₁ n : ℝ)) ^ (yscale A.C₁ n + 1) *
        (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) + 2) *
      (5 * (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) ≤ ε * Real.exp (ell2 n) := hP3c
  have hn0 : (0 : ℝ) < (n : ℝ) := by
    have : (0 : ℕ) < n := by omega
    exact_mod_cast this
  have h1n : (1 : ℝ) < (n : ℝ) := by
  ...
  rw [hnexp] at hP2'
  rw [hlogn] at hP3'
  have hP2nat : zscale A.C₁ n ^ (10 * Tscale n) ≤ n := by exact_mod_cast hP2'
  -- now the actual argument
  intro Q hQg hQT k hk0 hkL S hSp hSne hmodL hlt hub
  have hQ : ∀ q ∈ Q, q.Prime ∧ q ≤ zscale A.C₁ n ∧ SmoothUpTo (yscale A.C₁ n) (q - 1) :=
    fun q hq => mem_goodPrimes (hQg hq)
  have hpool : ∀ p ∈ S, p.Prime ∧ p ≤ xceil Q ∧ zscale A.C₁ n < p ∧
  ...
  have hxzT : xceil Q ≤ zscale A.C₁ n ^ (5 * Tscale n) := by
    calc xceil Q = Lmod Q ^ 5 := rfl
      _ ≤ (zscale A.C₁ n ^ Tscale n) ^ 5 := Nat.pow_le_pow_left hLzT 5
      _ = zscale A.C₁ n ^ (5 * Tscale n) := by rw [← pow_mul, mul_comm]
  ...
  have hcard3 : 3 ≤ S.card := by
    ...
    have hfin : (∏ p ∈ S, p) ≤ n := by
      calc (∏ p ∈ S, p) ≤ xceil Q ^ S.card := h1
        _ ≤ (zscale A.C₁ n ^ (5 * Tscale n)) ^ S.card := Nat.pow_le_pow_left hxzT _
        _ = zscale A.C₁ n ^ (5 * Tscale n * S.card) := (pow_mul _ _ _).symm
        _ ≤ zscale A.C₁ n ^ (10 * Tscale n) := by
  ...
  -- the overshoot bound
  have hlam : lambdaL Q ≤ zscale A.C₁ n ^ (yscale A.C₁ n + 1) :=
    lambdaL_le_pow (by omega) hQ
  have hZ0R : (0 : ℝ) < (zscale A.C₁ n : ℝ) := by
    have : (0 : ℕ) < zscale A.C₁ n := by omega
    exact_mod_cast this
  have hZ1R : (1 : ℝ) ≤ (zscale A.C₁ n : ℝ) := by exact_mod_cast hz1'
  have hlZ0 : (0 : ℝ) ≤ Real.log (zscale A.C₁ n : ℝ) := Real.log_nonneg hZ1R
  ...
  have hlogL : Real.log (Lmod Q : ℝ) ≤ (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) := by
  ...
  have hlamR : (lambdaL Q : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) := by
    exact_mod_cast hlam
  have hNstar : (Nstar Q : ℝ) ≤ (lambdaL Q : ℝ) * (1 + Real.log (Lmod Q : ℝ)) + 2 := by
    ...
    linarith
  have hNle : (Nstar Q : ℝ) ≤ (zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
  ...
  have hlogx : Real.log (xceil Q : ℝ)
      ≤ 5 * (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ) := by
  ...
  have hfin : (Nstar Q : ℝ) * Real.log (xceil Q : ℝ) ≤ ε * Real.log n := by
    calc (Nstar Q : ℝ) * Real.log (xceil Q : ℝ)
        ≤ ((zscale A.C₁ n : ℝ) ^ (yscale A.C₁ n + 1) *
            (1 + (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) + 2) *
          (5 * (Tscale n : ℝ) * Real.log (zscale A.C₁ n : ℝ)) :=
